...
import com.cocoa.rmq_client.bean.client.SearchParams;
...
import com.cocoa.rmq_client.bean.sql.CommitItem;
import com.cocoa.rmq_client.bean.taobao.TaobaoAllResp;
import com.cocoa.rmq_client.bean.taobao.TaobaoResp;
import com.cocoa.rmq_client.bean.taobao.TaobaoRespItem;
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                List<TaobaoRespItem> itemList = taobaoAllResp.tbk_item_get_response.results.n_tbk_item;
                CommitItem item = null;
                for (int i = 0; i < itemList.size(); i++) {
                    Thread.sleep(searchParams.getSleepTime() * 1000);
                    try {
                        // 查询数据，更新taobao 库
                        String id = itemList.get(i).getNum_iid();
                        item = new CommitItem();
